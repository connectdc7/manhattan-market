...
export type Product = {
  ...
  is_healthy?: boolean; // staff-toggled "healthy pick" flag — feeds the homepage's Product of the Day
  created_at?: string; // when the row was added — "new" for Product of the Day means recent here
};

// created_at values are staggered by hand below (rather than all defaulting
// to "now") so the local/no-Supabase preview has the same mix of
// long-standing and recently-added items that supabase/seed.sql seeds.
const daysAgo = (n: number) => new Date(Date.now() - n * 86_400_000).toISOString();

export const fallbackProducts: Product[] = [
  { id: "hot-coffee", name: "Fresh Brewed Coffee", category: "Hot Food", price: 2.25, stock: 24, blurb: "Hot, ready, and refilled all day.", swatch: "#6b4226", is_healthy: false, created_at: daysAgo(180) },
  { id: "bacon-egg-sandwich", name: "Bacon, Egg & Cheese", category: "Hot Food", price: 5.5, stock: 9, blurb: "Made fresh at the counter every morning.", swatch: "#c98b3a", is_healthy: false, created_at: daysAgo(180) },
  { id: "chicken-empanada", name: "Chicken Empanada", category: "Hot Food", price: 3.25, stock: 14, blurb: "Two in a bag, always warm.", swatch: "#d97b3f", is_healthy: false, created_at: daysAgo(120) },
  { id: "chips-classic", name: "Classic Potato Chips", category: "Snacks", price: 2.0, stock: 31, blurb: "The everyday bag by the register.", swatch: "#e0a938", is_healthy: false, created_at: daysAgo(180) },
  { id: "chocolate-bar", name: "Chocolate Bar", category: "Snacks", price: 1.75, stock: 42, blurb: "Impulse-buy shelf favorite.", swatch: "#5a3825", is_healthy: false, created_at: daysAgo(180) },
  { id: "trail-mix", name: "Trail Mix", category: "Snacks", price: 3.5, stock: 6, blurb: "Nuts, raisins, chocolate chips — a better-for-you snack.", swatch: "#8a6a3f", is_healthy: true, created_at: daysAgo(5) },
  { id: "cold-brew", name: "Bottled Cold Brew", category: "Drinks", price: 3.75, stock: 18, blurb: "Straight from the cooler.", swatch: "#3a2a1e", is_healthy: false, created_at: daysAgo(150) },
  { id: "orange-juice", name: "Orange Juice, 16oz", category: "Drinks", price: 2.5, stock: 22, blurb: "Squeezed, not from concentrate — 100% juice, no added sugar.", swatch: "#e8912a", is_healthy: true, created_at: daysAgo(2) },
  { id: "sparkling-water", name: "Sparkling Water", category: "Drinks", price: 1.5, stock: 2, blurb: "Almost out — restocking Thursday. Zero sugar, just bubbles.", swatch: "#7fa6a3", is_healthy: true, created_at: daysAgo(60) },
  { id: "energy-drink", name: "Energy Drink", category: "Drinks", price: 3.0, stock: 27, blurb: "The one everyone grabs before work.", swatch: "#3e6b4a", is_healthy: false, created_at: daysAgo(180) },
  { id: "milk-half-gallon", name: "Milk, Half Gallon", category: "Grocery", price: 3.25, stock: 11, blurb: "Whole, 2%, and skim in the cooler.", swatch: "#e8e4d8", is_healthy: true, created_at: daysAgo(90) },
...
// Toggles a product's "Healthy Pick" flag from the dashboard. Feeds the
// homepage's "Product of the Day" section (see lib/product-of-day.ts) —
// doesn't affect anything else about how the product is shown or sold.
export async function setProductHealthy(id: string, isHealthy: boolean): Promise<boolean> {
  const client = supabase;
  if (!client) return false;
  const { error } = await client.from("products").update({ is_healthy: isHealthy }).eq("id", id);
  if (error) {
    console.error("setProductHealthy:", error.message);
    return false;
  }
  return true;
}
